pipeline {
    ...
    stages{
       ...
       stage("Trigger CD Pipeline") {
            steps {
                script {
                    sh "curl -v -k --user clouduser:${JENKINS_API_TOKEN} -X POST -H 'cache-control: no-cache' -H 'content-type: application/x-www-form-urlencoded' --data 'IMAGE_TAG=${IMAGE_TAG}' ' http://ec2-13-233-114-241.ap-south-1.compute.amazonaws.com:8080/job/gitops-app-project-cd/buildWithParameters?token=gitops-token'"
                }
            }
       }
    }
